...
function updateChargeParticlesFromGrid(currentIter)
{
  ...
  const findCloudCell = (gx, preferredY) => {
    if (!chargeGridCloudMask)
      return {x : gx, y : preferredY};

    let bestX = gx;
    let bestY = -1;
    let bestScore = 0.0;
    for (let dx = -2; dx <= 2; dx++) {
      const searchX = (gx + dx + chargeGridW) % chargeGridW;
      for (let y = 1; y < chargeGridH - 1; y++) {
        const idx = chargeGridIndex(searchX, y);
        ...
        const lateral = 1.0 - Math.min(Math.abs(dx) / 3.0, 1.0);
        const score = mask * 0.65 + proximity * 0.25 + lateral * 0.10;
        if (score > bestScore) {
          bestScore = score;
          bestX = searchX;
          bestY = y;
