...
function updateChargeSeparationSystem(currentIter, chargeSources, iterScale = 1.0)
{
  ...
  for (let i = 0; i < chargeSources.length; i++) {
    ...
    if (cgScore > bestCGScore) {
      bestCGScore = cgScore;
      const randomCloudOffset = (Math.random() - 0.5) * clamp(0.015 + source.cloudDensity * 0.035, 0.015, 0.060);
      bestCG = {
        x : mod(source.x + randomCloudOffset, 1.0),
...
async function mainScript(initialBaseTex, initialWaterTex, initialWallTex, initialRainDrops)
{
  ...
  function setupDatGui(strGuiControls)
  {
    ...
    advanced_folder.add(guiControls, 'reduceLightningFlashes').name('Minimal lightning flash');
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.code == 'ControlRight') {
      ...
      guiControls.showWeatherStations = true;
      setWeatherStationsVisibility(true);
  ...
  */
  ...
  gl.uniform1f(gl.getUniformLocation(realisticDisplayProgram, 'minimalLightningFlash'), guiControls.reduceLightningFlashes ? 1.0 : 0.0);
